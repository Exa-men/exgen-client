...
const testimonials: TestimonialData[] = [
  ...
  {
    quote:
      "Dankzij Exa.men besparen we enorm veel tijd bij het voorbereiden van onze toetsen. De kwaliteit blijft hoog, terwijl we als docententeam meer tijd overhouden voor onze leerlingen.",
  ...
  {
    quote:
      "De adaptieve functionaliteit van Exa.men maakt gepersonaliseerd leren echt mogelijk. Onze leerlingen zijn enthousiaster en scoren gemiddeld 15% beter sinds we zijn overgestapt.",
    author: "Peter Bakker",
          ...
          >
            <CarouselContent className="-ml-2 md:-ml-4">
              {testimonials.map((testimonial, index) => (
                <CarouselItem
                  key={index}
                  className="pl-2 md:pl-4 py-8 md:py-10"
                >
              ...
              {testimonials.map((_, index) => (
                <button
                  ...
                  className={`h-2.5 rounded-full transition-all ${
                    current === index
                      ? "w-6 bg-examen-cyan"
                      : "w-2.5 bg-gray-300 hover:bg-examen-cyan/50"
                  }`}
